...
# Determine current python vesion
if sys.version > "4":
    PY_VER = None
elif sys.version > "3":
    PY_VER = 3
...
class Transparent_Window(QWidget):
    def __init__(self):
        ...
        self.ui.label_Drag.setAttribute(Qt.WA_TransparentForMouseEvents)
    ...
    def create_systray(self):
        ...
        self.tray_icon_menu.addAction(
            QAction("Hide 5 minutes", self, triggered=self.hideWindowForFiveMinutes))
    ...
    def hideWindowForFiveMinutes(self):
        self.hide()
        time.sleep(300)
        self.show()
    ...
    def get_printer_name(self):
        if sys.platform == 'win32':
            try:
                printer_name_raw = subprocess.check_output(
    ...
    def refresh_contents(self):
        computer_name = "<font color=white>" + self.get_computer_name(
        ) + "</font>"
        user_name = "<font color=white>" + getpass.getuser() + "</font>"
        network_adapter_name = "<font color=white>" + self.get_network_info(
            'card') + "</font>"
        ip_address = "<font color=white>" + self.get_network_info(
            'ip') + "</font>"
        printer_name = "<font color=white>" + self.get_printer_name(
        ) + "</font>"
